function BCI_testBinX2_manual(varargin)
                ...
                if isempty(slct) && (toc1-toc2) >= dtq
                    ...
                    if length(tempdat) >= 125                        
                        if obs_side ~= 0 % cued intervals can be used to update LDA parameter
                            [P,features,means_k] = testBCIX_manual(tempdat(:,1:8),Frfilt,Spfilt,lda_W,means_k,paramU);                                           
                            ...
                            [P,~,~] = testBCIX_manual(tempdat(:,1:8),Frfilt,Spfilt,lda_W,means_k,paramU);
                        end                        
                        disp('Predicted:')
                        P
